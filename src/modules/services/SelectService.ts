...
import { selectConfig } from "@/modules/config/selectConfig";

const fetchSelectOptions = async (key: string, query: any): Promise<any[]> => {
  const config = selectConfig[key];
  const { service, model, keyFilter, where = {}, mapToOption, select } = config;
...
export const selectService = {
  fetchSelectOptions(key: string, query: any): Promise<any[]> {
    return fetchSelectOptions(key, query);
  },
  createSelectOption(key: string, body: any, user: any): Promise<any> {
    return createSelectOption(key, body, user);
  },
};
