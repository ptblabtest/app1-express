...
const transformPipeline = (pipeline: any) => {
  ...
  const entities = [
    { type: "lead", data: prepared.lead },
    { type: "opportunity", data: prepared.opportunity },
    { type: "quote", data: prepared.quote },
    { type: "contract", data: prepared.contract },
  ].filter((entity) => entity.data !== null);

  const latestPipe = entities.reduce((latest, current) => {
    if (!latest.data || !current.data) return latest;
    const latestDate = new Date(latest.data.createdAt);
    const currentDate = new Date(current.data.createdAt);
    return currentDate > latestDate ? current : latest;
  }, entities[0] || { type: null, data: null });

  const dueDate = latestPipe.data?.dueDate || null;
...
export const pipelineService = {
  create: async (data: any, user: any) => {
    ...
    const result = await prisma.$transaction(async (tx: any) => {
      ...
      if (!stageType) {
        throw new Error(
          `No stage type found for pipeline with order ${stageOrder}`
        );
      }
      ...
      const pipeline = await tx.pipeline.create({
        data: {
          ...
          stages: {
            create: {
              stageTypeId: validatedData.stage?.stageTypeId || stageType.id,
...
const handleProgress = async (
  ...
  // Determine stage order for progression
  let stageOrder = 1; // default
  if (targetStage === "contract" || validatedData.contract) {
    stageOrder = 2;
  }
